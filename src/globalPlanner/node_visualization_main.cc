...
//========================================================================
/*!
\file    node_visualization_main.cc
...
using node_visualization::NodeVisualization;
using ros::Time;
...
NodeVisualization* node_visualization_ = nullptr;
...
void LaserCallback(const sensor_msgs::LaserScan& msg) {
  ...
  node_visualization_->ObservePointCloud(point_cloud_, msg.header.stamp.toSec());
  last_laser_msg_ = msg;
}
...
void OdometryCallback(const nav_msgs::Odometry& msg) {
  ...
  node_visualization_->UpdateOdometry(
...
void GoToCallback(const geometry_msgs::PoseStamped& msg) {
  ...
  printf("Goal: (%f,%f) %f\u00b0\n", loc.x(), loc.y(), angle);
  node_visualization_->SetNavGoal(loc, angle);
}
...
void LocalizationCallback(const amrl_msgs::Localization2DMsg msg) {
  ...
  node_visualization_->UpdateLocation(Vector2f(msg.pose.x, msg.pose.y), msg.pose.theta);
}
...
int main(int argc, char** argv) {
  ...
  // Initialize ROS.
  ros::init(argc, argv, "node_visualization", ros::init_options::NoSigintHandler);
  ros::NodeHandle n;
  node_visualization_ = new NodeVisualization(FLAGS_map, &n);
  ...
  while (run_ && ros::ok()) {
    ros::spinOnce();
    node_visualization_->Run();
    loop.Sleep();
  }
  delete node_visualization_;
  return 0;
}
